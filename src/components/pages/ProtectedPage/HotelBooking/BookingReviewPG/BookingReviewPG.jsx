            ...
            </Helmet>
            {
                ...
                <div className="bg-white my-10 sm:my-14 md:my-16 lg:my-20 shadow-md sm:shadow-lg py-5 xxs:py-8 sm:py-10 md:py-14 xs:px-4 sm:px-10 md:px-14 2xl:shadow-2xl relative mx-3 sm:mx-8 md:mx-auto md:max-w-screen-sm xl:max-w-screen-lg 3xl:max-w-screen-2xl">
                    ...
                    <div className=" mt-4 xxs:mt-7 sm:mt-10 3xl:mt-16 mb-3 xxs:mb-5 sm:mb-7 3xl:mb-10 px-2 xs:px-5">
                        <h4 className="text-primary font-semibold text-sm xs:text-base sm:text-lg xl:text-2xl"><span className="text-gray-700 text-sm xxs:text-base sm:text-lg xl:text-2xl">Hotel Name: </span>{order?.data?.hotelId?.hotelName}</h4>
                        <p className="text-gray-700 font-semibold mt-3 sm:mt-5">Booking No: <span className="font-normal">{order?.data?.bookingNo}</span></p>
                        <p className="text-gray-700 font-semibold mt-1">Booked Name: <span className="font-normal">{order?.data?.userId?.fullName}</span></p>
                        <p className="text-gray-700 font-semibold mt-1">Contact No: <span className="font-normal">{order?.data?.emergencyContact || order.data?.userId?.mobileNo}</span></p>
                                ...
                                <table className="w-full text-sm text-left text-gray-500">
                                    <tbody>
                                        <tr className="border-b w-[200px] whitespace-nowrap sm:whitespace-normal text-xs xs:text-sm sm:text-md xl:text-base">
                                            ...
                                            <th scope="col" className="px-3 sm:px-6 py-3 xxs:py-4 text-end font-semibold text-gray-900 w-5/12 sm:w-4/12 lg:w-3/12 xl:w-3/12 3xl:w-2/12">
                                                Quantity
                                            </th>
                                        </tr>
                                        {
                                            order?.data?.hotelId?.rooms?.map((d, i) => {
                                                if (order?.data?.selectedRooms?.[i]?.isSelected === true) {
                                                    return (
                                                        <tr key={i} className="border-b w-[200px] whitespace-nowrap sm:whitespace-normal text-xs xs:text-sm sm:text-md xl:text-base">
                                                            ...
                                                            <td className="px-3 sm:px-6 py-3 xxs:py-4 text-gray-800 text-end">
            ...
            }
            {
                ...
                <div className="bg-white my-10 sm:my-14 md:my-16 lg:my-20 shadow-md sm:shadow-lg py-5 xxs:py-8 sm:py-10 md:py-14 xs:px-4 sm:px-10 md:px-14 2xl:shadow-2xl relative mx-3 sm:mx-8 md:mx-auto md:max-w-screen-sm xl:max-w-screen-lg 3xl:max-w-screen-2xl">
                    ...
                    <div className=" mt-4 xxs:mt-7 sm:mt-10 3xl:mt-16 mb-3 xxs:mb-5 sm:mb-7 3xl:mb-10 px-2 xs:px-5">
                        <h4 className="text-primary font-semibold text-sm xs:text-base sm:text-lg xl:text-2xl"><span className="text-gray-700 text-sm xxs:text-base sm:text-lg xl:text-2xl">Hotel Name: </span>{order?.data?.hotelId?.hotelName}</h4>
                        <p className="text-gray-700 font-semibold mt-3 sm:mt-5">Booking No: <span className="font-normal">{order.data?.bookingNo}</span></p>
                        <p className="text-gray-700 font-semibold mt-1">Booked Name: <span className="font-normal">{PGuser ? order.data?.userId?.fullName : order.data?.userId?.name}</span></p>
                        <p className="text-gray-700 font-semibold mt-1">Contact No: <span className="font-normal">{PGuser ? order.data?.userId?.mobileNo : order.data?.emergencyContact}</span></p>
                        <p className="text-gray-700 font-semibold mt-1">Check In: <span className="font-normal text-red-500">{convertDate(order.data.checkIn)}</span></p>
                        <p className="text-gray-700 font-semibold mt-1">Check Out: <span className="font-normal text-red-500">{convertDate(order.data.checkOut)}</span></p>
                        <p className="text-gray-700 font-semibold mt-1">Guest: <span className="font-normal">Children: {order.data.childrenGuest}, Adult: {order.data.adultGuest}, Total: {order.data.totalGuest}</span></p>
                    ...
                    <form onSubmit={(e) => handleSubmit(e, order?.data?.isPaid)}>
                        {/* Refund policy agreement */}
                        <div className="mt-8 sm:mt-12 min-h-[1.5rem] pl-[1.5rem] flex justify-between items-center px-2">
                            <div className='flex items-center ml-2'>
                                <input
                        ...
                        <div className='mt-8 xxs:mt-10 sm:mt-16 text-end mr-3 xs:mr-0'>
                            {
                                loading ?
                                    ...
                                    :
                                    <button type="submit" className='inline-block rounded bg-primary px-3 py-2 xxs:px-4 xs:px-6 xxs:pb-2 xxs:pt-2.5 text-xs md:text-sm 2xl:text-base font-medium uppercase leading-normal text-gray-950 shadow-[0_4px_9px_-4px_#3b71ca] transition duration-150 ease-in-out hover:bg-primary-600 hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:bg-primary-600 focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:outline-none focus:ring-0 active:bg-primary-700 active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)]' data-te-ripple-init data-te-ripple-color="light">
                                        {(order?.data?.isPaid === true) ? 'Booked Again' : 'Proceed To Pay'}
                                    </button>
                            }
                        </div>
                    </form>
