...
import './PackageBookingReview.css';
import { useState } from "react";
import axios from "axios";
import { toast } from "react-toastify";

const PackageBookingReview = () => {
    ...
    const { data: singleOrderedPackage = [], isLoading } = useQuery(['singleOrderedPackage'], async () => {
        const res = await fetch(`${import.meta.env.VITE_clientSideLink}/api/package-orders/${orderPackageId}/${token ? 'user' : 'privilege_user'}`,
            {
                method: "GET",
                headers: {
    ...
    const handleSubmit = async (e) => {
        ...
        if (singleOrderedPackage?.data?.isOrder === true) {
            return;
        }

        try {
            setLoading(true);
            const response = await axios.post(`${import.meta.env.VITE_clientSideLink}/api/package-orders/payment/${orderPackageId}/${token ? 'user' : 'privilege_user'}`, { data: null }, {
                headers: {
                    authorization: `Bearer ${token ? token : pgToken}`,
                }
            });
    ...
    return (
        <div id="customScrollBar" className="bg-[#fbfbfb] py-[45px] xxs:py-[64px] lg:py-[74px] xl:py-[100px] 3xl:py-[106px]">
            <Helmet>
                <title>Package Booking Review | Royal Venture Limited</title>
            </Helmet>

            {
                singleOrderedPackage?.data === null ?
                    ...
                    <div className="bg-white my-10 sm:my-14 md:my-16 lg:my-20 shadow-md sm:shadow-lg py-5 xxs:py-8 sm:py-10 md:py-14 xs:px-4 sm:px-10 md:px-14 2xl:shadow-2xl relative mx-3 sm:mx-8 md:mx-auto md:max-w-screen-sm xl:max-w-screen-lg 3xl:max-w-screen-2xl">
                        ...
                        <div className=" mt-4 xxs:mt-7 sm:mt-10 3xl:mt-16 mb-3 xxs:mb-5 sm:mb-7 3xl:mb-10 px-2 xs:px-5">
                            <h4 className="text-primary font-semibold text-sm xs:text-base sm:text-lg xl:text-2xl"><span className="text-gray-700 text-sm xxs:text-base sm:text-lg xl:text-2xl">Package Code: </span>{singleOrderedPackage?.data?.packageId?.packageCode}</h4>
                            <h4 className="text-primary font-semibold text-sm xs:text-base sm:text-lg xl:text-2xl"><span className="text-gray-700 text-sm xxs:text-base sm:text-lg xl:text-2xl">Package Name: </span>{singleOrderedPackage?.data?.packageId?.packageName}</h4>
                            {
                                singleOrderedPackage?.data?.isPaid &&
                                <p className="text-gray-700 font-semibold mt-3 md:mt-5">Transaction Id: <span className="font-medium text-red-500">{singleOrderedPackage?.data?.transactionId}</span> <span className="font-medium text-green-500">(Paid)</span></p>
                            }
                            <p className="text-gray-700 font-semibold mt-3 sm:mt-5">Booking No: <span className="font-normal">{singleOrderedPackage?.data?.bookingNo}</span></p>
                            <p className="text-gray-700 font-semibold mt-1">Booked Name: <span className="font-normal">{token ? singleOrderedPackage?.data?.userId?.name : singleOrderedPackage?.data?.userId?.fullName}</span></p>
                            <p className="text-gray-700 font-semibold mt-1">Contact No: <span className="font-normal">{token ? singleOrderedPackage?.data?.emergencyContact : (singleOrderedPackage?.data?.emergencyContact || singleOrderedPackage?.data?.userId?.mobileNo)}</span></p>
                            <p className="text-gray-700 font-semibold mt-1">Guest: <span className="font-normal">Children: {singleOrderedPackage?.data?.childrenGuest}, Adult: {singleOrderedPackage?.data?.adultGuest}, Total: {singleOrderedPackage?.data?.totalGuest}</span></p>
                        </div>
                                ...
                                <tbody>
                                    ...
                                    <tr className="border-b w-[200px] whitespace-nowrap sm:whitespace-normal text-xs xs:text-sm sm:text-md xl:text-base">
                                        <td className="px-3 sm:px-6 py-3 xxs:py-4 text-gray-800">
                                            {convertDate(singleOrderedPackage?.data?.packageId?.startDate)}
                                        </td>
                                        ...
                                        <td className="px-3 sm:px-6 py-3 xxs:py-4 text-gray-800 text-right">
                                            {addCommas(parseFloat(singleOrderedPackage?.data?.packageId?.packageDiscountPrice))}/-
                                        </td>
                                    </tr>
                                    <tr className="bg-white border-b w-[200px] whitespace-nowrap xxs:whitespace-normal text-xs xs:text-sm sm:text-md xl:text-base">
                                        <td colSpan={4} className="px-3 sm:px-6 py-3 xxs:py-4 text-gray-950 font-bold text-right">
                                            {singleOrderedPackage?.data?.isPaid && <span>(<span className="font-medium text-green-500">Paid</span>)</span>}<span className="ms-2 mr-5">Total =</span> {addCommas(singleOrderedPackage?.data?.packageId?.packageDiscountPrice)}/-
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
...
export default PackageBookingReview;