...
const AddPG = () => {
    ...
    const formSubmit = (event) => {
        ...
        if (passwordStrength !== 'strong_password') {
            ...
            const pass = confirmPassword.current.value;
        ...
        <div className="bg-[#fbfbfb] w-[90%] mx-auto my-20 rounded-lg md:rounded-xl lg:rounded-2xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
            <div className="flex flex-col items-center py-10">
                ...
                <form ref={formRef} className='w-[90%] 2xl:w-[80%]' onSubmit={formSubmit}>
                    ...
                    {/* Mobile Number */}
                    <div className="relative z-0 w-full mb-6 xs:mb-10 group">
                        <input onKeyUp={handleNumberField} ref={number} type="text" name="floating_phone" id="floating_phone" className="block py-2.5 px-0 w-full text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
                        <label htmlFor="floating_phone" className="peer-focus:font-medium absolute text-gray-500 duration-300 transform -translate-y-6 scale-75 top-2 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Phone Number</label>
                        {
                    ...
                    <p className='text-[8px] md:text-sm text-gray-600 mt-6 sm:mt-10'>(<span className='text-red-500 text-[8px] md:text-sm'>*</span>) These fields are required.</p>
                </form>
            </div >
        </div >
    );
