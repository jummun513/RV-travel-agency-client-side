...
const UserReviews = () => {
    ...
    const handleSearchField = (event) => {
        ...
        const results = userReviews?.filter(d => (d.userId.fullName.toLowerCase().includes(keyword)));
    ...
    return (
        <div className="px-5 sm:px-10 py-7 xxs:pt-10 xxs:pb-14">
            <h2 className="text-center text-xl xs:text-3xl font-bold text-gray-800 xxs:mb-10">General Users Review</h2>
            {(userReviews || userReviews.length < 1) ?
                <div>
                    <div className="flex flex-col xxs:flex-row justify-between items-center">
                        <div className="w-2/5 me-5">
                            <p className="text-center xxs:text-left my-4 font-semibold text-gray-800 xxs:text-base xs:text-xl">Total : {searchKeyword ? searchData?.length : userReviews?.length}</p>
                        </div>
                        <div className="w-3/5">
                            <form>
                                <div className="flex">
                    ...
                    <div className="mt-10">
                        <div className="relative overflow-x-auto sm:overflow-x-hidden shadow-md sm:rounded-lg lg:min-w-[720px]">
                            <table className="w-full text-sm text-left text-gray-500">
                                ...
                                <tbody>
                                    {
                                        (searchKeyword ? searchData : userReviews)?.map((d, i) => {
                                            return (
