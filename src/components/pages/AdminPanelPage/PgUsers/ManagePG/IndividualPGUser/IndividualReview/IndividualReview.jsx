...
const IndividualReview = ({ singlePGUser }) => {
    ...
    const postReview = async (event) => {
        ...
        setLoading(true);
        try {
            const response = await axios.post(`${import.meta.env.VITE_clientSideLink}/api/reviews/admin/add-new`, { ...data, userId: singlePGUser?._id }, {
                headers: {
                    ...
                    <div className="py-8 px-5 w-[30rem] rounded-lg shadow-lg">
                        <div className="flex items-center">
                            <div className="mr-2 sm:mr-4 w-8 xxs:w-12 lg:w-14 2xl:w-16 rounded-full ring-2 ring-primary">
                                <img className="rounded-full" src={singlePGUser?.avatar.length > 0 ? singlePGUser?.avatar?.[0]?.thumbnailUrl : userImg} alt={singlePGUser?.fullName + ' image'} />
                            </div>
                            <div>
                                <p className="text-gray-800 font-semibold">{singlePGUser?.fullName}</p>
                                <p className="text-gray-700">{singlePGUser?.occupation}</p>
                            </div>
                        </div>
