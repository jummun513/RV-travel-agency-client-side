...
const AddHotel = () => {
    ...
    const handleUpload = async (event) => {
        ...
        const formattedCompanyName = name.replace(/\s+/g, '_');
        ...
        if (data.images.length > 0 && data.thumbnail.length > 0 && data.rooms.length > 0 && error === '') {
            ...
            try {
                ...
                uploadImage(data.thumbnail, `Royal_Venture/Hotels/${formattedCompanyName}/thumbnail`).then(d => uploadedData.thumbnail = d).then(
                    await uploadImage(uploadedData.images, `Royal_Venture/Hotels/${formattedCompanyName}/images`).then(d => uploadedData.images = d).then(
                        data.rooms?.map(async (item, i) => await uploadImage(item.pictures, `Royal_Venture/Hotels/${formattedCompanyName}/rooms_${i}`).then(d => { return uploadedData.rooms[i].pictures = d }))
                    )
                )
