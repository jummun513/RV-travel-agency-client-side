...
const AddToHotOffer = () => {
    ...
    const [data, setData] = useState({ offerEnd: Date.now() });
    ...
    const handleSubmit = async (event) => {
        ...
        if (error === '' && data?.photo) {
            try {
                ...
                await uploadImage(data.photo, `Royal_Venture/Hot_offers`).then(d => uploadedData.photo = d);

                const response = await axios.post(`${import.meta.env.VITE_clientSideLink}/api/hot-offers/add-new`, uploadedData, {
                    headers: {
                ...
                if (response.data === 'success') {
                    ...
                    notify();
                    navigate('/admin-panel/manage-hot-offers');
                }
                else {
                    notifyError();
                }
    ...
    return (
        <div className="mx-10 px-5 2xl:px-10 pt-14 pb-10 my-14 rounded-lg shadow-[0_3px_10px_rgb(0,0,0,0.2)]">
            <h2 className="text-center text-gray-800 font-bold text-4xl 3xl:text-5xl">Add To Hot Offers</h2>
            <form ref={formRef} className="mt-14" onSubmit={handleSubmit} >
                ...
                {/* offer end date */}
                <div className="mt-10">
                    <label className="block mb-2 text-sm font-medium text-gray-900">Offer Ends <sup className="text-red-500">*</sup></label>
                    <ReactDatePicker
                        ...
                        showYearDropdown
                        dropdownMode="select"
                        selected={data?.offerEnd}
                        timeInputLabel="Time:"
                        showTimeInput
                        onChange={(e) => handleInputChange('offerEnd', e)}
                        className='lg:w-[30rem] 2xl:w-[45rem] 4xl:w-[50rem] mt-1 input input-bordered input-info text-gray-950 bg-white'
                    />
                </div>

                {/* offer desc name */}
                <div className="mt-10">
                    <label className="block mb-2 text-sm font-medium text-gray-900">Offers Description <sup className="text-red-500">*</sup></label>
                ...
                <div className="mt-10">
                    ...
                    <input onChange={(e) => handleInputChange('target', e)} type="text" className="shadow-sm bg-gray-50 border border-gray-400 text-gray-900 text-sm rounded-lg focus:border-primary focus:outline-none block w-full p-2.5" placeholder="https://www.theroyalventure.com/package-tour/details/65d047aa7484ddbfea15e2ae" required />
                </div>
